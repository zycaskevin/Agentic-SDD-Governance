...
For a controlled installation, download the wheel and `SHA256SUMS.txt` from `v0.2.0rc1`, select exactly one universal wheel, and let `sha256sum` or `shasum` verify it before installation. Do not ask a human to copy, paste, or visually approve a digest.
...
- A manual workflow builds once, validates source contracts, smoke-tests only the new wheel, publishes through the `testpypi` OIDC environment, downloads the exact version back from TestPyPI for a second fresh-environment smoke test, and publishes the same artifact to PyPI only when explicitly selected.
...
4. Configure GitHub environments `testpypi` and `pypi` with environment-scoped PyPI Trusted Publishers. Require a human approval on the `pypi` environment so promotion can be decided after the TestPyPI round trip succeeds.
...
- MyHermes or any patient/customer/payment system still requires an environment-specific key ceremony, Broker rehearsal, synthetic/staging pilot, incident drill, and separate authorization before real data.
