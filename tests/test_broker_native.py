...
from sddgov.broker import (
    ...
    receive_broker_health_response,
)
...
class NativeBrokerTests(unittest.TestCase):
    ...
    def test_cleanup_claim_boundary_preserves_connectable_replacement(self):
        socket_path = self.root / "approval.sock"
        parent_descriptor, staging_descriptor = self._directories(socket_path)
        replacement = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        inserted = False
        ...
        try:
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as server:
                ...
                def replace_after_claim_stat(directory: int, name: str):
                    nonlocal inserted
                    metadata = broker_module._stat_at(directory, name)
                    if name.startswith(".broker-cleanup-") and not inserted:
                        replacement.bind(str(socket_path))
                        replacement.listen(1)
                        inserted = True
                    return metadata
                ...
                _unlink_socket_if_identity(
                    ...
                    socket_path.name,
                    identity,
                    stat_at=replace_after_claim_stat,
                )
    ...
    def test_atomic_publication_race_does_not_clobber_final_socket(self):
        ...
        def publish_after_replacement(*args):
            nonlocal inserted
            if not inserted and args[3] == socket_path.name:
                inserted = True
                replacement.bind(str(socket_path))
                replacement.listen(1)
