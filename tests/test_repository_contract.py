...
class RepositoryContractTests(unittest.TestCase):
    ...
    def test_packaged_hard_gate_assets_match_canonical_sources(self):
        paths = (
            ...
            "services/sddgov-broker.newsyslog.conf",
    ...
    def test_current_repo_installed_governance_is_healthy_and_current(self):
        ...
        self.assertEqual(report["managed_file_count"], 72)
    ...
    def test_rc1_work_package_and_rollback_bind_current_proof(self):
        ...
        self.assertIn(
            "- Evidence: `DEP-RC1-REVIEW-FIX-R12-020` (authoritative)",
            work_package,
        )
        self.assertIn(
            "Complete and strictly verify `DEP-RC1-REVIEW-FIX-R12-020`",
            work_package,
        )
        rollback = (
    ...
    def test_broker_service_unit_is_hardened(self):
        ...
        self.assertIn("<key>ThrottleInterval</key>", launchd)
        self.assertIn("<integer>30</integer>", launchd)
        self.assertIn("<key>GroupName</key>\n  <string>_sddgov</string>", launchd)
        self.assertIn("<key>Umask</key>\n  <integer>7</integer>", launchd)
        rotation = ROOT / "services/sddgov-broker.newsyslog.conf"
        self.assertTrue(rotation.is_file())
        rows = [
            line.split()
            for line in rotation.read_text(encoding="utf-8").splitlines()
            if line and not line.startswith("#")
        ]
        self.assertEqual(
            rows,
            [[
                "/var/log/sddgov-broker.log",
                "root:_sddgov",
                "640",
                "7",
                "1024",
    ...
    def test_l3_runbook_pins_authority_and_validates_launchd_assets(self):
        ...
        self.assertIn("/etc/newsyslog.d/sddgov-broker.conf", runbook)
