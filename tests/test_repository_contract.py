...
from sddgov import _require_supported_python
...
class RepositoryContractTests(unittest.TestCase):
    ...
    def test_source_validation_checks_every_embedded_governance_asset(self):
        with patch("sddgov.cli._resource_files", return_value={"VERSION": b"tampered\n"}):
    ...
    def test_rc1_work_package_and_rollback_bind_current_proof(self):
        ...
        self.assertIn(
            "- Evidence: `DEP-RC1-REVIEW-FIX-R10-018` (authoritative)",
            work_package,
        )
        self.assertIn(
            "Complete and strictly verify `DEP-RC1-REVIEW-FIX-R10-018`",
            work_package,
        )
        rollback = (
