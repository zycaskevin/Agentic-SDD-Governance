...
class RepositoryContractTests(unittest.TestCase):
    ...
    def test_source_validation_requires_broker_and_pilot_runtime_modules(self):
        ...
        for missing in ("broker.py", "pilot.py"):
            with self.subTest(missing=missing), patch.object(
                ...
                "is_file",
                autospec=True,
                side_effect=lambda path, missing=missing: (
                    False
    ...
    def test_english_readme_exposes_first_run_and_governance_tables(self):
        ...
        self.assertIn("agentic-sdd-governance==0.2.0rc1", readme)
    ...
    def test_redaction_inventory_matches_sensitive_identifier_contract(self):
        ...
        self.assertIn("MAX_REDACTION_FILE_BYTES = 10 * 1024 * 1024", redactor)
        self.assertIn("STREAM_CHUNK_BYTES = 64 * 1024", redactor)
    ...
    def test_rc1_work_package_and_rollback_bind_current_proof(self):
        ...
        self.assertIn(
            "- Evidence: `DEP-RC1-REVIEW-FIX-R9-017` (authoritative)",
            work_package,
        )
        self.assertIn(
            "Complete and strictly verify `DEP-RC1-REVIEW-FIX-R9-017`",
            work_package,
        )
        rollback = (
    ...
    def test_release_workflow_is_manual_isolated_and_attested(self):
        ...
        workflow = yaml.load(  # noqa: S506 -- BaseLoader preserves workflow keys as strings.
        ...
        self.assertNotIn("needs", workflow["jobs"]["build-and-smoke"])
        ...
        self.assertIn(
            "refs/tags/v0.2.0rc1", workflow["jobs"]["build-and-smoke"]["if"]
        )
        self.assertIn(
            "refs/tags/v0.2.0rc1", workflow["jobs"]["publish-testpypi"]["if"]
        )
        self.assertEqual(
        ...
        self.assertNotIn(
            "inputs.publish_pypi",
            workflow["jobs"]["publish-github-release"]["if"],
        )
        ...
        self.assertIn(
            "refs/tags/v0.2.0rc1", workflow["jobs"]["verify-testpypi"]["if"]
        )
    ...
    def test_broker_service_mirrors_are_byte_identical(self):
        for name in ("sddgov-broker.service", "com.sddgov.broker.plist"):
    ...
    def test_rollback_runbook_fetches_live_protected_branch(self):
        ...
        fetch = runbook.index("git fetch --prune origin main")
