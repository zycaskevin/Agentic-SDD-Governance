...
class RepositoryContractTests(unittest.TestCase):
    ...
    def test_shareable_evidence_contains_no_absolute_user_workspace_paths(self):
        exposed = []
        for artifact in sorted((ROOT / "evidence").glob("DEP-*/shareable/artifacts/*")):
            try:
    ...
    def test_rc1_work_package_and_rollback_bind_current_proof(self):
        ...
        self.assertIn(
            "- Evidence: `DEP-RC1-REVIEW-FIX-R7-015` (authoritative)",
            work_package,
        )
        self.assertIn(
            "Complete and strictly verify `DEP-RC1-REVIEW-FIX-R7-015`",
            work_package,
        )
        rollback = (
    ...
    def test_every_tracked_proof_dep_is_portable_strict(self):
        ...
        for dep in sorted((ROOT / "evidence").glob("DEP-*")):
            ...
            if not summary.is_file():
                if not any(entry.name != "private" for entry in dep.iterdir()):
                    continue
    ...
    def test_release_workflow_is_manual_isolated_and_attested(self):
        ...
        systemd = (ROOT / "services/sddgov-broker.service").read_text(encoding="utf-8")
        for required in (
