...
class BrokerTests(unittest.TestCase):
    def setUp(self):
        bound_identity = patch(
            "sddgov.broker._bound_socket_identity",
            side_effect=lambda server: (1, server.socket_path.inode),
        )
        bound_identity.start()
        self.addCleanup(bound_identity.stop)
    ...
    def test_caller_cannot_substitute_the_fixed_approver_store(self):
        ...
        with patch.dict(
            os.environ,
            {"SDDGOV_TRUSTED_APPROVERS_FILE": "/tmp/attacker-selected.json"},
