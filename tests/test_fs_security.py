...
class FilesystemSecurityTests(unittest.TestCase):
    def test_file_set_snapshot_reapplies_a_smaller_bound_to_cached_bytes(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            artifact = root / "artifact.bin"
            artifact.write_bytes(b"x" * (300 * 1024))

            with FileSetSnapshot(root, "bounded cache") as snapshot:
                self.assertEqual(
                    len(
                        snapshot.read(
    ...
    def test_new_file_applies_the_exact_mode_independent_of_umask(self):
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "public-receipt.json"
    ...
    def test_new_file_rejects_and_removes_a_foreign_uid_generation(self):
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "public-receipt.json"
            real_fstat = os.fstat
            ...
            with (
                patch("sddgov.fs_security.os.fstat", side_effect=foreign_regular_file),
                self.assertRaisesRegex(ValueError, "custody contract"),
            ):
                write_new_regular_file(
                    output,
                    b"public receipt\n",
