...
class MonorepoRollbackBenchmarkTests(unittest.TestCase):
    ...
    def test_scaled_setup_timeout_applies_to_every_fixture_git_command(self):
        timeout = setup_timeout_seconds(50_000)

        def fake_git(_root, *arguments, **kwargs):
            self.assertEqual(kwargs.get("timeout"), timeout, arguments)
            return "synthetic-sha" if arguments[:2] == ("rev-parse", "HEAD") else ""

        with patch(
            "scripts.benchmark_monorepo_rollback.setup_timeout_seconds",
            return_value=timeout,
        ), patch(
            "scripts.benchmark_monorepo_rollback._git",
            side_effect=fake_git,
        ), patch(
            "scripts.benchmark_monorepo_rollback.rollback_ref_is_cleanly_revertible",
            return_value=True,
        ):
            report = run_benchmark([1], 1)

        self.assertTrue(report["cases"][0]["all_proofs_passed"])
