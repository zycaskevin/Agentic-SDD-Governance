...
class TrustedRunnerContractTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        def synthetic_control_plane_loader(path: Path, _label: str):
            candidate = Path(path)
            if candidate == TRUSTED_APPROVERS_FILE:
                candidate = self.trust_file
            if candidate not in {self.trust_file, self.runtime_context_path}:
                raise ValueError("synthetic control-plane path is not allowlisted")
            return json.loads(candidate.read_text())

        self.control_plane_loader_patch = patch(
            "sddgov.autonomy.load_control_plane_json",
            side_effect=synthetic_control_plane_loader,
        )
...
"""
    ...
    def test_approval_consumer_uses_fixed_authority_matching_runner_binding(self) -> None:
        descriptor, bundle = self._sealed_bundle()
        request = self._request(bundle, nonce="nonce-authority-snapshot")
        configured_loader = autonomy_module.load_control_plane_json
        observed_paths: list[Path] = []

        def observed_loader(path: Path, label: str):
            observed_paths.append(Path(path))
            return configured_loader(path, label)
        ...
        try:
            ...
            os.close(descriptor)
        self.assertEqual(result["outcome"], "completed")
        self.assertIn(TRUSTED_APPROVERS_FILE, observed_paths)
