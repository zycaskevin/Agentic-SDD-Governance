...
from sddgov.fs_security import write_new_regular_file
...
def _case(file_count: int, repeats: int) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="sddgov-monorepo-bench-") as temporary:
        ...
        _git(root, "init", "--quiet", timeout=setup_timeout)
        _git(
            root,
            "config",
            "user.email",
        ...
        base_sha = _git(root, "rev-parse", "HEAD", timeout=setup_timeout)
        ...
        rollback_ref = _git(
            root, "rev-parse", "HEAD", timeout=setup_timeout
        )
        ...
        _git(
            ...
            "--quiet",
            "-m",
            "bind synthetic evidence",
            timeout=setup_timeout,
        )
        reviewed_head_sha = _git(
            root, "rev-parse", "HEAD", timeout=setup_timeout
        )
