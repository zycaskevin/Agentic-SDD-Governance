...
def _receive_broker_health(client: socket.socket) -> bytes:
    expected = b"READY\n"
    response = bytearray()
...
def _terminate_broker_process(
    process: subprocess.Popen[str], *, timeout_seconds: int = 10
) -> tuple[str, str]:
    """Signal a Broker and drain both pipes while waiting for termination."""
            ...
            if socket_path.exists():
                try:
                    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client:
                        ...
                        if _receive_broker_health(client) != b"READY\n":
                            raise RuntimeError(
...
@contextmanager
def _fresh_smoke_temporary_directory():
    """Create a short AF_UNIX-capable workspace with an explicit fallback."""
    ...
    for parent in candidates:
        ...
        try:
            temporary = tempfile.TemporaryDirectory(prefix="sgw-", dir=parent)
        except OSError as exc:
            failures.append(type(exc).__name__)
            continue
        root = canonicalize_platform_path(Path(temporary.name))
        socket_limit = 103 if sys.platform == "darwin" else 107
        if os.name == "posix" and len(
            os.fsencode(root / "installed-broker.sock")
        ) > socket_limit:
            temporary.cleanup()
            failures.append("AF_UNIX path limit")
            continue
        try:
            ...
            temporary.cleanup()
        return
    detail = ", ".join(failures) if failures else "no supported parent"
...
def smoke(
    ...
    with _fresh_smoke_temporary_directory() as temporary:
        root = Path(temporary)
