...
from sddgov.fs_security import canonicalize_platform_path, remove_owned_at
from sddgov.redaction import LOCAL_USER_PATH_PATTERN
...
def _absolute_without_symlink_resolution(path: Path) -> Path:
    return canonicalize_platform_path(path.expanduser())
...
def open_or_create_directory_tree(
    path: Path,
    label: str,
    *,
    mode: int = 0o755,
) -> OpenedDirectory:
    """Open or create a POSIX directory tree without following symlinks."""
    _require_posix_release_files()
    expanded = _absolute_without_symlink_resolution(path)
    flags = (
        os.O_RDONLY
...
def mask_local_paths(value: str) -> str:
    """Remove host-specific temporary and home paths from public text."""
    rendered = LOCAL_USER_PATH_PATTERN.sub("<local-path>", value)
    for prefix in sorted(
        {tempfile.gettempdir(), str(Path.home())}, key=len, reverse=True
    ):
        normalized = prefix.rstrip(os.sep)
        if not normalized or normalized == os.sep:
            continue
        escaped_prefix = re.escape(normalized)
        escaped_separator = re.escape(os.sep)
        rendered = re.sub(
            rf"{escaped_prefix}"
            rf"(?=$|{escaped_separator}|[\s:;,'\"])"
...
def write_new_public_report(path: Path, rendered: str) -> None:
    """Create a report through descriptor-bound, no-follow operations."""
    expanded = _absolute_without_symlink_resolution(path)
    _safe_direct_name(expanded.name, "release report")
    with open_or_create_directory_tree(
        expanded.parent, "release report directory"
    ) as directory:
        directory.write_bytes(expanded.name, rendered.encode("utf-8"), mode=0o644)
