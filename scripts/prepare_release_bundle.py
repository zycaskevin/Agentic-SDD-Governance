...
def _public_error(value: str) -> str:
    """Remove host-specific local paths from a shareable failure report."""
...
def main() -> int:
    ...
    try:
        ...
        report = {"ok": False, "error": _public_error(str(exc))}
    rendered = json.dumps(report, ensure_ascii=False, indent=2) + "\n"
    if args.report is not None:
        try:
            write_new_public_report(args.report, rendered)
        except (OSError, ValueError) as exc:
            report = {
                "ok": False,
                "error": _public_error(f"cannot write release report: {exc}"),
            }
            rendered = json.dumps(report, ensure_ascii=False, indent=2) + "\n"
    print(rendered, end="")
