...
def _locked_requirements(lock: Path) -> dict[str, tuple[Version, set[str]]]:
...
def _wheel_identity(path: Path, label: str) -> tuple[str, Version]:
    try:
...
def _sdist_identity(path: Path) -> tuple[str, Version]:
    try:
...
def prepare_release_bundle(
    ...
    lock = _regular_file(lock, "governance lock")
    ...
    if (
        ...
        raise ValueError("project wheel and source archive version must match the release")
    ...
    for dependency in dependency_wheels:
        ...
        if locked_row is None or dependency_version != locked_row[0]:
            raise ValueError("dependency wheel is not an exact locked requirement")
        if _sha256(dependency) not in locked_row[1]:
            raise ValueError("dependency wheel digest is absent from the locked hashes")
