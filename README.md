...
From a source checkout with its development virtual environment:
...
For controlled or offline installation, download the wheel and registry checksum from the matching GitHub Release, then let the machine verify the digest:
...
(cd sdg-release && sha256sum -c wheel.SHA256SUMS)
...
On macOS, replace the checksum command with `shasum -a 256 -c wheel.SHA256SUMS --strict`. Private Release downloads require an authenticated GitHub CLI. Never paste a token or checksum into chat for human validation.
...
- Redaction reduces accidental disclosure but does not certify legal anonymization.
