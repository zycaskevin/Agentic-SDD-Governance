...
The predeclared investigation threshold is the verifier-owned constant: p95 greater than 5 seconds or any incorrect proof result. The CLI intentionally has no threshold override. Crossing it means profile and investigate the exact-tree implementation; it does not authorize replacing Base-tree equality with an affected-path-only assertion.
