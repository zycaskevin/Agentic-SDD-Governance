...
```bash
BENCHMARK_RUN_DIR="$(mktemp -d)"
PYTHONPATH=src python scripts/benchmark_monorepo_rollback.py \
  --file-counts 1000,10000,50000 \
  --repeats 3 \
  --output "$BENCHMARK_RUN_DIR/result.json"
```

The report writer creates a new file exclusively and refuses existing files or
symlinked path components. Review the new result before promoting it through a
separate governed change; do not write directly over the tracked
`latest-result.json` snapshot.
