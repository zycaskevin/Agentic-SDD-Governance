...
- `tests/test_evidence_flow.py`: artifact mutation/deletion, complete raw/report coverage, Collector/media binding, unknown-extension/HAR disguise, traversal DEP ID, retained-dirfd Evidence-root creation, Collector input, Collector/Redactor/Verifier zones, attach verification snapshot, custom output, atomic manifest/report/summary parent replacement, hardlinks, symlinked source/destination/control files, duplicate collector label, and malformed Evidence contracts.
...
- `python -m pytest -q` — 139/139 passed on macOS outside the restricted sandbox, including fragmented real Unix-socket protocol, extra-byte denial, and all base/source/DEP/zone/output parent-replacement assertions.
...
- Existing L0/L1 autonomous routing, Checkpoint behavior, artifact integrity, Reviewer bootstrap/signing, Merge digest, CI Cost Guard, install/doctor/uninstall, redaction rules, benchmark harness, adapters, and packaged-resource parity all passed in the same 139-test suite.
