...
`PYTHONPATH=src python -m pytest tests/ -q` completed with 120/120 passing tests after the independent Review findings were converted to adversarial cases. Repository validation, CI Cost Guard inspection/local gate, Python compile, full local and portable DEP verification, isolated wheel/sdist build, fresh-wheel Hermes install/doctor, and the offline synthetic Muse pilot all passed. They remain subject to one final exact-Head rerun before Review receipt signing.

## Before/after evidence

Before, the released/candidate verifier accepted synthetic artifact mutation/deletion, omitted raw rows, extension/HAR bypass, path escape side effects, link traversal, duplicate overwrite, caller-created Owner roots, caller-asserted L2 freshness, cross-clone L3 replay, and Reviewer-key resurrection. After, each case is a deterministic regression test that fails closed while normal L0/L1 paths continue.
...
- L3 `CONTINUE` requires a separately deployed root-owned atomic nonce broker; without it SDG returns machine-actionable `BLOCKED`. This repository does not claim a clone-local file provides global single use.
