...
13. A single source file is limited to 10 MiB and a logical text line to 1 MiB characters. Larger inputs fail before publication and must be recollected as a bounded excerpt or safe summary. Eligible UTF-8 text is decoded and redacted in 64 KiB chunks; logical lines and private-key state span chunk boundaries so a credential split by an I/O boundary cannot bypass a rule. Unterminated private-key blocks fail closed.
