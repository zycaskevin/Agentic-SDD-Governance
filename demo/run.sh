...
if [[ -f "$repo_root/src/sddgov/cli.py" ]]; then
  if [[ -x "$repo_root/.venv/bin/python" ]] && python_is_supported "$repo_root/.venv/bin/python"; then
    ...
    printf '%s\n' 'Python 3.10 or newer is required to run the offline source demo.' >&2
    exit 2
  fi
  ...
  if [[ -x "$repo_root/.venv/bin/python" ]] && python_is_supported "$repo_root/.venv/bin/python"; then
    ...
    printf '%s\n' 'Python 3.10 or newer is required to render the offline demo result.' >&2
    exit 2
  fi
