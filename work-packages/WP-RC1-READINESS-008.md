...
- Complete and strictly verify `DEP-RC1-REVIEW-FIX-R15-023`; generate a local PR attachment block only from redacted shareable artifacts. R15 records the independent PR #36 review that rejected the Broker's FD/path inode assumption; R14 records the predecessor release, cleanup, and macOS logging remediation; R13 records the independent PR #34 review that rejected the rollback topology and isolated release-test environment. Earlier packages remain immutable predecessor evidence. A collector UUID or redacted numeric review suffix is not used as GitHub identity.
