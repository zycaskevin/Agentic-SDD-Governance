...
- Evidence: `DEP-RC1-REVIEW-FIX-R7-015` (authoritative); `DEP-RC1-REVIEW-FIX-R6-014` and `DEP-RC1-REVIEW-FIX-R5-013` are immutable predecessor packages.
...
- Complete and strictly verify `DEP-RC1-REVIEW-FIX-R7-015`; generate a local PR attachment block only from redacted shareable artifacts. R7 records the exact PR #28 GraphQL thread IDs and `discussion_r...` URLs; R6 and R5 remain predecessor evidence rather than current authority. A collector UUID or redacted numeric review suffix is not used as GitHub identity.
