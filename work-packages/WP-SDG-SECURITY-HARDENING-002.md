...
- Bind Collector identity, immutable source suffix, and detected media type; retain verified base/source/DEP/zone/output directory descriptors through DEP creation, final Evidence reads, verification, attachment, and control-document mutation. Attachment output must consume the exact verified control snapshot, bind it to a framed SHA-256 generation in content and default filename, stage before its final linearization check, and publish atomically without clobber. Pre-check swaps fail closed; later control generations stay cryptographically distinct; later destination writers are preserved.
