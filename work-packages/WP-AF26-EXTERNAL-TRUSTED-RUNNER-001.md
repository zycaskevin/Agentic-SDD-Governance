...
- Evidence：`DEP-AF26-EXTERNAL-TRUSTED-RUNNER-001`、
  ...
  `DEP-AF26-PR-REVIEW-FIX-001`、`DEP-AF26-MERGE-ROLLBACK-NORMALIZATION-001`
...
- Status：Rehearsal Runner、production hard-deny、Schema、Agent Factory 0.3 串接、
  ...
  修復，本機 256-test Local Green與 fresh Wheel／doctor通過。Merge Gate multi-commit rollback
  atomicity正由 `DEP-AF26-MERGE-ROLLBACK-NORMALIZATION-001` 以 append-only、final-tree-invariant
  方式修復；受信簽章、replacement hosted run、Merge與部署尚未完成；production authority
  未放寬，且不宣稱真實 L3。
