...
class FileSetSnapshot:
    ...
    def read(self, relative: PurePosixPath, *, max_bytes: int) -> bytes:
        ...
        if existing is not None:
            if len(existing["raw"]) > max_bytes:
                raise ValueError("descriptor snapshot input exceeds the bounded size")
            existing["max_bytes"] = min(existing["max_bytes"], max_bytes)
            return existing["raw"]
    ...
    def file_metadata(self, relative: PurePosixPath) -> os.stat_result:
        """Return metadata for one retained file generation already read."""
        record = self._records.get(str(relative))
        if record is None or record["file_fd"] < 0:
            raise ValueError(f"{self.label} file snapshot is not retained")
        return os.fstat(record["file_fd"])
...
def write_new_regular_file(
    ...
    *,
    ...
    required_file_uid: int | None = None,
    require_parent_group_inheritance: bool = False,
) -> None:
    ...
    try:
        ...
        if (
            ...
            required_parent_uid is not None
            and parent_metadata.st_uid != required_parent_uid
        ):
            raise ValueError(
                f"{label} parent does not match the required custody contract"
            )
        descriptor = os.open(
            expanded.name,
            os.O_WRONLY
        ...
        # ``open(..., mode)`` is filtered by the process umask. Public
        # handoff artifacts need the reviewed exact mode, so set and verify it
        # on the retained descriptor before any bytes become durable.
        ...
        if (
            ...
            or (
                required_file_uid is not None
                and created.st_uid != required_file_uid
            )
            or (
                require_parent_group_inheritance
                and created.st_gid != parent_metadata.st_gid
            )
        ):
            raise ValueError(
                f"{label} must be a single-linked regular file matching the custody contract"
            )
        ...
        if (
            ...
            or (
                required_file_uid is not None
                and (
                    final_fd.st_uid != required_file_uid
                    or final_path.st_uid != required_file_uid
                )
            )
        ):
        ...
        if (
            ...
            or (
                ...
                committed_path.st_uid,
                committed_path.st_gid,
            ...
            or (
                required_file_uid is not None
                and (
                    committed_fd.st_uid != required_file_uid
                    or committed_path.st_uid != required_file_uid
                )
            )
        ):
        ...
        if identity is not None:
            try:
                remove_owned_at(
                    ...
                    identity,
                    label,
                    expected_size=owned_size,
                    expected_digest=hashlib.sha256(data[:owned_size]).hexdigest(),
                )
            except BaseException as exc:
                if cleanup_error is None:
                    cleanup_error = exc
