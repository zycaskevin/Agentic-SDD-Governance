...
RULES: tuple[Rule, ...] = (
    ...
    Rule("password", re.compile(r'''(?i)((?:"|')?\b(?:password|passwd)\b(?:"|')?\s*[=:]\s*)("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\s,;"']+)'''), r"\1[REDACTED_PASSWORD]"),
    Rule("secret-field", re.compile(r'''(?i)((?:"|')?\b(?:secret|api[_-]?key|access[_-]?token|refresh[_-]?token|client[_-]?secret)\b(?:"|')?\s*[=:]\s*)("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\s,;"']+)'''), r"\1[REDACTED_SECRET]"),
