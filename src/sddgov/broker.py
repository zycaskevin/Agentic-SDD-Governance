...
def _bound_socket_identity(server: socket.socket) -> tuple[int, int]:
    metadata = os.fstat(server.fileno())
    if not stat.S_ISSOCK(metadata.st_mode):
        raise ValueError("L3 Broker bind did not create a Unix socket descriptor")
...
def serve_broker(socket_group: str) -> None:
    ...
    try:
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as server:
            ...
            bound_socket_identity = _bound_socket_identity(server)
            ...
            if (
                ...
                raise ValueError("L3 Broker socket path changed immediately after bind")
