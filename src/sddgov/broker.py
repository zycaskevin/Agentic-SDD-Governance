...
def _valid_ledger_record(value: Any) -> bool:
    ...
    if (
        ...
        or not 12 <= len(nonce) <= 256
        or any(ord(character) < 0x21 or ord(character) > 0x7E for character in nonce)
        or not isinstance(value.get("receipt_sha256"), str)
        or SHA256_PATTERN.fullmatch(value["receipt_sha256"]) is None
        or not isinstance(value.get("operation_payload_sha256"), str)
        or SHA256_PATTERN.fullmatch(value["operation_payload_sha256"]) is None
...
def handle_request(raw: bytes, ledger: NonceLedger) -> bytes:
    ...
    if (
        ...
        or not 12 <= len(nonce) <= 256
        or any(ord(character) < 0x21 or ord(character) > 0x7E for character in nonce)
        or not isinstance(request.get("receipt_sha256"), str)
        or SHA256_PATTERN.fullmatch(request["receipt_sha256"]) is None
        or not isinstance(request.get("operation_payload_sha256"), str)
        or SHA256_PATTERN.fullmatch(request["operation_payload_sha256"]) is None
    ):
        return b"REJECTED\n"
...
def _socket_errors() -> list[str]:
    ...
    if (
        ...
        errors.append("L3 Broker socket must be root-owned and not world-writable")
    return errors
...
def broker_readiness(root: Path) -> dict[str, Any]:
    ...
    check(
        ...
        else (_ for _ in ()).throw(ValueError("L3 Broker requires Linux or macOS")),
    )
    check(
        "agent_identity_separation",
        lambda: {"euid": os.geteuid()}
        if hasattr(os, "geteuid") and os.geteuid() != 0
        else (_ for _ in ()).throw(ValueError("Agent process must not run as root")),
    )
...
def serve_broker(socket_group: str) -> None:
    ...
    try:
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as server:
            ...
            os.chown(L3_NONCE_BROKER, 0, group_id, follow_symlinks=False)
            os.chmod(L3_NONCE_BROKER, 0o660, follow_symlinks=False)
            socket_metadata = L3_NONCE_BROKER.lstat()
            if (
            ...
            while not shutdown_requested:
                ...
                with connection:
                    ...
                    try:
                        ...
                        # A client may disconnect after sending a request. Its broken
