...
def _require_record_sha256(row: Any, expected: str, label: str) -> None:
    """Require one explicit SHA-256 RECORD binding without attribute errors."""
    hash_value = getattr(row, "hash", None)
    if (
        hash_value is None
        or getattr(hash_value, "mode", None) != "sha256"
        or getattr(hash_value, "value", None) != expected
    ):
        raise ValueError(f"{label} is not hash-bound by wheel RECORD")
...
def _require_owner_distribution(
    ...
    _require_record_sha256(
        launcher_rows[0],
        _record_sha256(expected_launcher),
