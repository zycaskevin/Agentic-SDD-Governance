...
The trusted approver key store and audience binding are outside the repository at fixed paths `/etc/sddgov/trusted-approvers.json` and `/etc/sddgov/trusted-approver-domains.json`; the sidecar binds each key to the canonical repository ID, exact host-local repository root, and trust domain, so candidate-controlled Git configuration alone cannot select authority. On macOS the verifier maps only the platform-owned `/etc` alias to physical `/private/etc`, while arbitrary symlinks remain forbidden. An `SDDGOV_TRUSTED_APPROVERS_FILE` caller override fails closed instead of selecting authority. All persistent control-plane files—the ledger, runtime context, key store, and repository/trust-domain sidecar—are root-owned and not writable by group or other. The Unix socket must be root-owned with exact mode `0660` and the fixed dedicated socket group (`sddgov` on Linux/WSL2, `_sddgov` on macOS) so approved non-root Agent service accounts can connect. A different group or any broader/narrower mode is `NOT_READY`. The Agent process must be non-root.
...
3. Create root-owned `/etc/sddgov/runtime-context.json`, `/etc/sddgov/trusted-approvers.json`, and `/etc/sddgov/trusted-approver-domains.json` as `root:sddgov` mode `0640` (or `root:root` mode `0644`). The selected mode must let the approved non-root Agent accounts read all fixed control-plane files without granting write access. Mode `0600` is not valid for this deployment because it prevents the Agent-side verifier from loading the runtime context and approver stores. The private key never goes on this host unless it is itself the approved external signer.
...
3. Create `/etc/sddgov` as `root:_sddgov` mode `0750`, then create the runtime context, fixed trusted approver key store, and fixed trusted approver domain sidecar as `root:_sddgov` mode `0640` (or `root:wheel` mode `0644`). The selected mode and every parent directory must let approved non-root `_sddgov` Agent accounts traverse the path and read all three files without write access; mode `0600` is not valid for these Agent-read control files.
...
The required clock-safety interval is five minutes. Before rollover, confirm the Broker host and every authorized signer are synchronized to the approved time source and remain within that bound. The rollover deadline is the latest `expires_at` among every receipt issued in the current epoch plus five minutes; if the complete issuance set or clock bound cannot be proven, do not roll the ledger.

With the Broker stopped after that deadline, validate and preserve the old ledger as a root-owned, immutable audit archive, move it to a unique same-filesystem epoch name under change control, fsync the directory, and let the Broker create a new owner-only ledger. Record the old ledger digest, final record count, last receipt issuance time, latest receipt `expires_at`, five-minute safety interval, calculated rollover deadline, clock-source evidence, archive path, new epoch start, and readiness result. Never restore an archived or older epoch as the active ledger. This bounds active scan memory and latency without treating archive deletion as replay protection; receipt signature, exact payload, runtime context, and expiry checks remain mandatory before Broker consumption.
