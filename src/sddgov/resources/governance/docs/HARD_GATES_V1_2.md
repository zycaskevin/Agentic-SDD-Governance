...
5. `CONTINUE` requires `/usr/local/libexec/sddgov-approval-broker`, a root-owned external control plane, to atomically consume the nonce across every clone and machine. SDG checks every parent, opens the executable without following links, verifies the opened inode, and executes that descriptor. The Agent process itself must be non-root so root ownership remains a separate identity. A missing or untrusted context/broker returns `BLOCKED` without another human approval prompt. Clone-local `decisions.json` is audit state, never the single-use authority ledger.
