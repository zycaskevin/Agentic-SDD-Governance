...
For L2, the Main Agent generates and validates a request that fixes the exact canonical assumption paths, Owner key ID, validity, repository, trust domain, and reviewed Owner-client source digest, then uses `sddgov decision show-product-approval`; it never asks the Owner to edit JSON, handle a digest, expose a key, paste a signature, or review code. The exact request JSON is itself one signed assumption, and the same client binding occurs exactly once inside the other decision contract. Request and assumption artifacts are read and reconciled as one retained snapshot. The separate root-controlled audience sidecar binds the Owner key to the canonical repository ID, exact host-local repository root, and trust domain; candidate Git configuration alone is not authority. Before any interpreter or import starts, the complete Owner venv, launcher, package, metadata, `pyvenv.cfg`, terminal, and signer channel must be controlled by an identity the Agent cannot modify. Runtime isolated-mode, RECORD, mode, and site checks are diagnostics, not proof against pre-check execution. The signer socket/channel is inaccessible to Agent processes; generic SSH confirmation alone is not semantic card binding. The Owner initiates the isolated client and rejects unexpected prompts, or an external signer displays and binds the exact card/receipt itself. The client reads one foreground controlling-terminal A/B line; only A may approve and B always declines. It writes a public `0640` receipt to a pre-provisioned Owner-write-only/Agent-read-only outbox, so the Main Agent can import it without chat relay or broader signer access. Local Green then runs exact `sddgov decision verify-product` to recheck the recursive-unique envelope, stored row, active key, expiry, audience, exact request, scope, assumptions, current client identity, summary, and validity. The Main Agent must not invoke the Owner entry point or answer its prompt.
