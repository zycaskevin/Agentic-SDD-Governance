...
For L2, the Main Agent generates and validates a request that fixes the exact canonical assumption paths, Owner key ID, validity, repository, trust domain, and reviewed Owner-client source digest, then uses `sddgov decision show-product-approval`; it never asks the Owner to edit JSON, handle a digest, expose a key, or paste a signature. The same client binding must occur exactly once inside the signed decision assumption contract. The separate root-controlled audience sidecar binds the Owner key to the canonical repository ID, exact host-local repository root, and trust domain; candidate Git configuration alone is not authority. The installed `sddgov-owner` launcher must enter isolated Python before importing `sddgov`, reject loader injection, byte-match the packaged launcher and wheel RECORD/entry-point contract, and run by absolute path from an Owner-controlled independently installed venv with system-site packages disabled and a working directory outside the Agent repository. It shows the exact bounded A/B scope from `/dev/tty`; only A may approve and B always declines without signing. It verifies the installed source identity against the signed binding and audience sidecar, reconstructs the Base-compatible receipt and assumptions, and delegates only to the matching confirmation-constrained external Ed25519 signer. The Main Agent imports the output, while the repository Local Green contract runs exact `sddgov decision verify-product` to recheck the stored signature, row, active key, expiry, audience, scope, assumptions, current installed-client identity, and validity. It must not invoke the Owner entry point, answer its prompt, or treat chat text or a schema-shaped decision row as authority.
