...
from . import autonomy as autonomy_module
from .autonomy import evaluate_escalation, import_operation_approval
from .trust import TRUSTED_APPROVERS_FILE
...
class TrustedRunner:
    ...
    def _consume_approval_locked(self, request: _ValidatedRequest) -> None:
        ...
        authority_document = _strict_json_bytes(authority_raw, "trusted_approvers")
        try:
            fixed_authority = autonomy_module.load_control_plane_json(
                TRUSTED_APPROVERS_FILE,
                "fixed trusted approver store",
            )
        except (ValueError, OSError) as exc:
            raise TrustedRunnerViolation("approval_verification_failed") from exc
        if _canonical_hash(fixed_authority) != authority_hash:
            raise TrustedRunnerViolation("approval_authority_identity_changed")
