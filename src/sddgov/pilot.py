...
def run_quick_demo(output: Path | None = None) -> dict[str, Any]:
    ...
    required_evidence_keys = (
        ...
        "agent_install_ok",
        "text_redaction_ok",
        "binary_image_fail_closed",
        "strict_dep_ok",
    )
    missing = [key for key in required_evidence_keys if key not in evidence_pilot]
    if missing:
        raise ValueError(
            "synthetic Muse result is missing required check(s): " + ", ".join(missing)
        )
    required_checks = {
