...
def _load_root_owned_control_plane_json(path: Path, label: str) -> dict[str, Any]:
    """Read one fixed root-owned control-plane document by retained descriptors."""
    candidate = canonicalize_platform_path(path.expanduser())
...
def load_control_plane_json(path: Path, label: str) -> dict[str, Any]:
    """Load authority for a non-root Agent from an independent root identity.

    A root-owned file is not a separate authority boundary when the Agent itself
    runs as root. The Agent-facing verifier therefore retains its explicit root
    refusal even though the isolated Owner client has a distinct fixed-path
    reader below.
    """
    ...
    return _load_root_owned_control_plane_json(path, label)


def load_owner_runtime_control_plane_json(path: Path, label: str) -> dict[str, Any]:
    """Load one public trust document for the isolated root Owner client only.

    The Owner client is deliberately root-owned and root-executed, so it cannot
    use the Agent-facing loader's separate-identity rule. This entry point keeps
    the same descriptor, ownership, link, size, duplicate-JSON, and path-chain
    checks while additionally requiring root execution and one of the two fixed
    public trust paths. It never accepts a caller-selected authority location.
    """
    if os.name == "nt" or not hasattr(os, "geteuid") or os.geteuid() != 0:
        raise ValueError(f"{label} requires the isolated root Owner runtime")
    candidate = canonicalize_platform_path(path.expanduser())
    allowed = {
        canonicalize_platform_path(TRUSTED_APPROVERS_FILE),
        canonicalize_platform_path(TRUSTED_APPROVER_DOMAINS_FILE),
    }
    if candidate not in allowed:
        raise ValueError(f"{label} must use one fixed Owner trust path")
    return _load_root_owned_control_plane_json(candidate, label)
