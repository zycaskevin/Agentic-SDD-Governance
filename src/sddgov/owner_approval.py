...
from .trust import load_owner_runtime_control_plane_json
...
OWNER_OUTBOX_MODE = 0o2750
OWNER_RECEIPT_MODE = 0o640
...
def _trusted_owner_public_key(root: Path, approver_id: str) -> bytes:
    approver = _trusted_approver(
        root,
        approver_id,
        control_plane_loader=load_owner_runtime_control_plane_json,
    )
    try:
...
def build_product_approval_card(
    ...
    with FileSetSnapshot(root, "product approval card inputs") as snapshot:
        ...
        binding = _trusted_approver_domain(
            root,
            card["approver_id"],
            control_plane_loader=load_owner_runtime_control_plane_json,
        )
    ...
    try:
        ...
        if (
            ...
            and (
                ...
                or stat.S_IMODE(parent_metadata.st_mode) != OWNER_OUTBOX_MODE
            )
        ):
            raise ValueError(
                "signed product approval receipt parent is not a pre-provisioned one-way handoff"
            )
        try:
            metadata = os.stat(
                expanded.name,
            ...
            if (
                ...
                or stat.S_IMODE(metadata.st_mode) != OWNER_RECEIPT_MODE
            ):
                raise ValueError(
                    "signed product approval receipt output has unsafe ownership or mode"
                )
            exists = True
        require_directory_path_identity(
            expanded.parent,
...
def _existing_product_approval(
    ...
    with FileSetSnapshot(
        ...
        relative = PurePosixPath(expanded.name)
        raw = snapshot.read(
            relative,
            max_bytes=MAX_APPROVAL_REQUEST_BYTES,
        )
        parent_metadata = os.fstat(snapshot.root_fd)
        leaf_metadata = snapshot.file_metadata(relative)
        if (
            parent_metadata.st_uid != os.geteuid()
            or stat.S_IMODE(parent_metadata.st_mode) != OWNER_OUTBOX_MODE
            or leaf_metadata.st_uid != os.geteuid()
            or leaf_metadata.st_gid != parent_metadata.st_gid
            or leaf_metadata.st_nlink != 1
            or stat.S_IMODE(leaf_metadata.st_mode) != OWNER_RECEIPT_MODE
        ):
            raise ValueError(
                "existing signed product approval receipt has an unsafe handoff contract"
            )
    ...
    if (
        ...
        != timedelta(days=card["valid_days"])
    ):
        raise ValueError(
            "existing signed product approval receipt validity differs from the displayed card"
        )
    options = {
...
def approve_product_decision(
    ...
    *,
    ...
    verified, receipt_sha256 = _verify_product_envelope(
        root,
        envelope,
        control_plane_loader=load_owner_runtime_control_plane_json,
    )
    ...
    write_new_regular_file(
        ...
        directory_mode=0o700,
        required_parent_mode=OWNER_OUTBOX_MODE,
        required_parent_uid=os.geteuid(),
        required_file_uid=os.geteuid(),
        require_parent_group_inheritance=True,
    )
    return {
        "state": "APPROVED",
        "decision_id": receipt["decision_id"],
