...
1. Create a dedicated system group `sddgov` and add only approved Agent service accounts. The unit's `Group=` value must remain identical to the `--socket-group` value: bind then creates the socket with its final group, the Broker skips an unnecessary `chown`, and the empty capability set may safely omit `CAP_CHOWN`.
...
The repository provides `services/com.sddgov.broker.plist` and bounded log-rotation policy `services/sddgov-broker.newsyslog.conf`. The plist runs as `root:_sddgov`, applies decimal `Umask` 7 (octal `0007`), and keeps both streams in `/var/log/sddgov-broker.log`.
...
5. Install `services/sddgov-broker.newsyslog.conf` as `/etc/newsyslog.d/sddgov-broker.conf` with `root:wheel` ownership and mode `0644`; run `newsyslog -n -f /etc/newsyslog.d/sddgov-broker.conf` and confirm the preview retains seven compressed 1 MiB rotations. Pre-create `/var/log/sddgov-broker.log` as `root:_sddgov` mode `0640`, or let launchd create it under the plist's `0007` umask. Do not bootstrap without this bounded-retention policy.
...
7. Start a new login session, unset the retired trust-store environment override, and run `broker doctor` from the non-root Agent account.
