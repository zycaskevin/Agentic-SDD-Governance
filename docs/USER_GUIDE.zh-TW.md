...
```bash
test ! -e .venv-sddgov  # 必須使用全新、不存在的目錄
...
```bash
SDDGOV_BIN="$(pwd)/.venv-sddgov/bin/sddgov"  # 方法 C 的 checkout 請改用 "$(pwd)/.venv/bin/sddgov"
...
開始 Red 之前，先啟動 repository 已安裝的 `agentic-sdd-governance` Skill，讓本節的 Evidence、風險分級、核准與 Gate 路由在整個修復工作階段持續生效；不得只摘錄下方命令而跳過 Skill contract。
