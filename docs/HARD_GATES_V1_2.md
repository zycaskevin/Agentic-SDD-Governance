...
The accepted rollback v2 fields are `rollback_action: git_revert`, `rollback_ref: <full 40-character lowercase commit SHA>`, `verify_action: python_module`, and `verify_module: pytest` or `unittest`. The verifier resolves the ref as an existing commit and requires it to be strictly after the exact Base and at or before the reviewed candidate Head. Symbolic, abbreviated, missing, Base/out-of-range, and future refs fail closed. These values describe a bounded action; the verifier never executes free-form Markdown or shell text. Because a commit cannot contain its own SHA, create the implementation commit first and bind its SHA in a later evidence commit before review.
