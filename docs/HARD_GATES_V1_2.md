...
The accepted rollback v2 fields are `rollback_action: git_revert`, `rollback_ref: <full 40-character lowercase commit SHA>`, `verify_action: python_module`, and `verify_module: pytest` or `unittest`. The verifier resolves the ref as an existing single-parent commit and requires it to be strictly after the exact Base and at or before the reviewed candidate Head. Symbolic, abbreviated, missing, merge, Base/out-of-range, and future refs fail closed. It then uses Git's tree-level three-way merge simulation to prove that reverting the commit at the exact reviewed Head is conflict-free; it does not execute the Markdown command, hooks, a shell, or network access. These values describe a bounded action. Because a commit cannot contain its own SHA, create an implementation-only commit first, do not modify any of its paths in later commits, and bind its SHA in a later Evidence commit before review. Gate and Receipt commits remain audit-only descendants.

During the trusted-verifier v1-to-v2 bootstrap only, the parser recognizes one exact legacy record whose command is `git revert --no-edit <full 40-character lowercase candidate commit SHA>` and whose verification is `python -m pytest`. Candidate validation enables this bridge only when the exact Base is `f44cb5f4897f6c821f817fcf178581b43777163a` and the selected path is `evidence/DEP-SDG-SECURITY-HARDENING-EXP8-001/rollback.md`; every later Base rejects v1. The field set and values must match exactly; unknown standalone text, duplicates, additional fields, wrappers, chaining, alternate commands, and placeholders fail closed. The verifier treats these strings as data and never executes them. New rollback records use v2.
