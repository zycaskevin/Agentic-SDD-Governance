...
17. Under `team-standard`, L2 is one bounded plain-language Owner choice recorded by the Agent and reopened only when its scope or recorded assumptions change; it never requires a Terminal, digest, signer, Broker, or cryptographic receipt. In this release, `solo-fast` and `regulated` retain the separate-identity signed L2 receipt path rather than accepting an Agent-recorded plain-language choice. L3 binds the complete canonical operation payload and is consumed once.
...
19. Routine review pre-authorization never covers Private Repo content, Secrets, credentials, raw Evidence, unredacted sensitive material, Production dumps, real user data, a new vendor or destination, new login/MFA/OAuth scope, account permission or other access, or new cost. An exact recorded Private Repo/Reviewer decision is a separate authorization path. Automated third-party review never replaces a signed independent protected-file Review receipt.
20. Merge and Release Readiness are effect-free L1 engineering channels with zero Owner operations. If Merge would trigger Production, public publication, Secret/IAM, payment, destructive, irreversible, or other external effects, separate readiness from one canonical L3 operation; an L1/L2 baseline decision never authorizes that execution.
21. `EXTERNAL_REVIEW_IS_NOT_A_SINGLE_POINT_OF_AUTHORITY`: valid findings from a configured automated reviewer must be repaired and rechecked. A provider status that explicitly skipped review is not review. After one automatic attempt on an exact revision, provider skip or unavailability is recorded and the signed independent review receipt, full Merge Gate, and hosted CI form the bounded fallback; do not retry indefinitely or route the outage through the Owner.
