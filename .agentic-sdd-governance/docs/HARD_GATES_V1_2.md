...
For L2, the receipt follows `schemas/product-decision-approval-receipt.schema.json` and binds the decision ID, exact scope, a non-empty list of repository-relative assumption artifacts with their SHA-256 values, the canonical assumptions-list digest, reopen condition, owner identity, validity window, and nonce. Import it with `sddgov decision import-product-approval`. On every reuse, SDG reopens the signed artifact paths and recalculates their hashes; a missing, linked, moved, or changed artifact reopens the decision. Caller-supplied digest or `reopen_condition_triggered` booleans are not authority. `decision record` is a deprecated fail-closed command and cannot create approval.
...
4. The signed receipt includes `operation_payload`: repository, project, environment, scope, exact category, target, non-secret parameters, and sensitive-effect flags. `sddgov autonomy evaluate request.json --path .` re-verifies the stored signed envelope and its digest under the decision lock, then compares the complete canonical payload SHA-256, operation ID, signer, expiry, and nonce. Editing `.sddgov/decisions.json` cannot create or expand L3 authority.
5. `CONTINUE` requires `/usr/local/libexec/sddgov-approval-broker`, a root-owned external control plane, to atomically consume the nonce across every clone and machine. A missing or untrusted broker returns `BLOCKED` without another human approval prompt. Clone-local `decisions.json` is audit state, never the single-use authority ledger.
...
Strict DEP verification does not trust `manifest.json` assertions. It reopens every available artifact without following symlinks and recomputes normalized path, file type, link count, byte size, and SHA-256. It rejects missing files, unregistered extras, hardlinks, duplicate manifest paths, duplicate collector destinations, path escape, symlinked DEP zones/files/control documents, and altered redaction associations. Every raw row must appear exactly once in either `report.files.source` or `report.blocked.file`; every shareable row must appear exactly once as an output. Unknown or binary types and HAR remain blocked rather than relying on filename-controlled generic text redaction.
