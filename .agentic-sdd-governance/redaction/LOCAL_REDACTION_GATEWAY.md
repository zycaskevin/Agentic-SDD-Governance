...
8. Collector and redaction operations retain verified directory descriptors through source reads, same-directory temporary writes, and atomic replacement; a pathname check followed by a pathname reopen is not sufficient. Verification must then recompute output size and SHA-256 and match the exact source-to-output association.
