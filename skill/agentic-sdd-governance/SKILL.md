...
1. Read `core/POLICY_KERNEL.md` from the governed repository.
2. Read exactly one selected file from `profiles/`.
3. Read the current Work Package, SDD references, and existing decisions.
4. Classify the work L0-L3 before mutation.
5. Continue autonomously for approved L0/L1 work. Escalate only within the Policy Kernel boundary.
...
4. Read only the relevant collector file named by `references/collector-routing.md`.
5. Read `redaction/LOCAL_REDACTION_GATEWAY.md` before any artifact may leave local storage.
