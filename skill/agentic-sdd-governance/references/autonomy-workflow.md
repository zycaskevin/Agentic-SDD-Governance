...
Every known action request must include an explicit `effects` object. Use `{}` only after classification confirms that no Production, public-publication, destructive, irreversible, Secret, permission-boundary, real-payment, or high-privilege effect applies.
...
Merge and Release Readiness use exact L1 requests with empty effects and require zero Owner operations. If either declares a real effect, classify it as one canonical L3 operation instead of attaching an approval to the engineering step. Actual Production deploy and public release always require exact L3 operation payloads after every readiness guard passes; no L1 or L2 baseline decision authorizes execution.
