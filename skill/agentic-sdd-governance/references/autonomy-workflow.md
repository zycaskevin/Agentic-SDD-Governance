...
Routine L0/L1 requests use an exact authority-free envelope. They must not carry `approval_id`, `operation_id`, `operation_payload`, `decision_id`, `decision_scope`, or `decision_package`. If an inner payload, target, parameter, or inferred effect contradicts the outer category/risk/effects, classify from the higher-risk facts and fail closed instead of accepting the caller label.
