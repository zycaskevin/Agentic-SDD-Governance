...
When the contract sets `hosted.post_merge_verification` to `manual_only`, reject automatic `push` triggers. The Ready PR check is the Work Package run; a Release verification is an explicit `workflow_dispatch`, not a hidden second run after Merge.
