...
For L2, the Main Agent generates and validates the governed request, then uses `sddgov decision show-product-approval`; it never asks the Owner to edit JSON, handle a digest, expose a key, or paste a signature. The separate `sddgov-owner approve-product` entry point must run from an Owner-controlled terminal and an independently installed reviewed artifact. It shows the exact A/B scope, reads the choice from `/dev/tty`, reconstructs the receipt and assumptions, and delegates only to the matching confirmation-constrained external Ed25519 signer. The Main Agent imports the verified output automatically. It must not invoke the Owner entry point, answer its prompt, or treat chat text as the signed receipt.
